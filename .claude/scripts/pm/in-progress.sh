...
if [ -d ".claude/epics" ]; then
  for updates_dir in .claude/epics/*/updates/*/; do
    ...
    if [ -f "$updates_dir/progress.md" ]; then
      ...
      # Get task name from the Epic-prefixed task file
      # Find the Epic-prefixed task file that has this GitHub issue number
      task_file=$(find ".claude/epics/$epic_name/" -name "[A-Z][A-Z][A-Z][0-9][0-9][0-9].md" -exec grep -l "issues/$issue_num" {} \; 2>/dev/null | head -1)
      if [ -f "$task_file" ]; then
